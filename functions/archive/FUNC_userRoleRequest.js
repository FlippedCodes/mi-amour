  ...
  if (requestedRole) {
    ...
    switch (allowed) {
      case true:
        ...
        break;
      case false:
        messageFail(reaction.message, `\`${requestedRole.name}\` is a 18+ role. You are not old enough, that I can give you this role!`, user.tag);
        break;
      case null:
        messageFail(reaction.message, `\`${requestedRole.name}\` is a 18+ role. You haven't applied for that yet. Have a read of <#${config.info_channelID}> how to get access.`, user.tag);
        break;
      default:
        break;
    }
